...
******************************************************************/
const greaterThanOrEqualTo25 = temperatures.filter((temperature)=>temperature>=25);
console.log(greaterThanOrEqualTo25)

const lessThan20 = temperatures.filter((temperature)=>temperature < 20);
...
const temperaturesFahrenheit = temperatures.map(temp => (temp * 9/5) + 32);
...
const highestTemperature = temperatures.reduce((max, temp) => {
    if (temp > max) {
      return temp;
    } else {
      return max; 
...
const lowestTemperature = temperatures.reduce((min, temp) => {
    if (temp < min) {
      return temp; 
    } else {
      return min; 
